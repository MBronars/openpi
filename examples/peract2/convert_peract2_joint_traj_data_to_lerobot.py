...
def main(push_to_hub: bool = False):
    ...
    output_path = "/data/user_data/mbronars/lerobot_datasets/peract2_traj"
        ...
        for ep in tqdm(episodes):
                ...
                for i, k in enumerate(key_frames[:-1]):
                    ...
                    action = np.concatenate([
                        demo[next_k].left.joint_positions,
                        [demo[next_k].left.gripper_open],
                        demo[next_k].right.joint_positions,
                        [demo[next_k].right.gripper_open]])
                    
                    # print alert of the gripper open or close is not 0 or 1:
                    if state[7] not in [0, 1]:
                        print(f"Warning: gripper open state is not 0 or 1: {state[7]}")
                    if state[15] not in [0, 1]:
                        print(f"Warning: gripper open state is not 0 or 1: {state[15]}")
