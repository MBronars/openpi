...
def make_libero_example() -> dict:
    ...
    return {
        ...
        "observation/action": np.random.rand(16),
...
class HiveformerSubgoalInputs(transforms.DataTransformFn):
    """
    ...
    def __call__(self, data: dict) -> dict:
        ...
        if "tokenized_subtask" in data:
            inputs["tokenized_subtask"] = data["tokenized_subtask"]
            inputs["tokenized_subtask_mask"] = data["tokenized_subtask_mask"]
...
@dataclasses.dataclass(frozen=True)
class HiveformerOutputs(transforms.DataTransformFn):
    """
...
@dataclasses.dataclass(frozen=True)
class HiveformerSubgoalOutputs(transforms.DataTransformFn):
    """
    This class is used to convert outputs from the model back the the dataset specific format. It is
    used for inference only.
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # For Libero, we only return the first 7 actions (since the rest is padding).
        # For your own dataset, replace `7` with the action dimension of your dataset.
        return {"actions": np.asarray(data["actions"][:, :8]), "language_tokens": np.asarray(data["subgoals"][0]), "segmentation": np.asarray(data["subgoals"][1])}
